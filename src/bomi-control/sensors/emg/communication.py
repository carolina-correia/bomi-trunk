...
def create_bin_command(mode=1,nch=0,fsamp=2,start=1):
    rec = 0
    trig = 0
    ext = 0
    hpf = 1
    ...
    if (not number_of_channels or not sample_frequency or not bytes_in_sample):
        raise Exception("Could not set number_of_channels and/or bytes_in_sample")

    command_in_bytes = int(command).to_bytes(2, byteorder="big")
    # print("number_of_channels", number_of_channels) # it is 16 now
...
def connect_to_sq(serverIP,port,mode,nch,fsamp):
    """
    ...
    start_command, nb_channels, sample_freq, bytes_per_ch = create_bin_command(mode,nch,fsamp,start=1)
...
def read_raw_bytes(nb_channels,bytes_per_ch,conn):
...
def bytes_to_integers(nb_channels,bytes_per_ch,sample_from_channels_as_bytes,output_milli_volts):
    channel_values = []
...
def getSignals_old(conn,nb_samples,bytes_per_ch,adapter_chs):
    """
    Convert bytes to integers per channel and to mV
    """
    buffer_size = nb_samples * bytes_per_ch # 600 * 3 = 1800
    sample_len = adapter_chs * bytes_per_ch #12*3 = 36
    emg_chs = adapter_chs - 4
    emg_len = emg_chs * bytes_per_ch #24
    data_bytes = conn.recv(buffer_size*3)
    # print("DDD",nb_samples, bytes_per_ch,emg_chs, buffer_size)

    emg_tw = []
    for j in range(0, buffer_size, sample_len): # every block of 36 samples 
        emg_bytes = data_bytes[j:j+emg_len]
        if len(emg_bytes)>0:
            for i in range(0, len(emg_bytes), bytes_per_ch):
                ch_bytes = emg_bytes[i:i+bytes_per_ch]
                ch_int = convert_bytes_to_int(ch_bytes, bytes_per_ch)
                ch_mV = np.float64(ch_int * CONVERSION_FACTOR)
                emg_tw = np.append(emg_tw,ch_mV)

    emg_tw = np.reshape (emg_tw, (int(len(emg_tw)/emg_chs) , emg_chs))
    # print('test2: ', np.shape(emg_tw))  # should have 50 x 8
    return emg_tw
...
def disconnect_from_sq(conn,mode,nch,fsamp):
    """
    Disconnect from Sessantaquattro by sending a stop command
    """
    if conn is not None:
        start = 0
        stop_cmd, _,_,_ = create_bin_command(mode,nch,fsamp,start)
        conn.send(stop_cmd)
        conn.shutdown(2)
        conn.close()
    else:
